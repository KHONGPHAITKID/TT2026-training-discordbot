...
class StatsCog(commands.Cog):
    ...
    def _build_help_embed(self) -> discord.Embed:
        ...
        embed.add_field(
            ...
            "• `/show_question` – re-post the active question\n"
        ...
        embed.add_field(
            name="Models",
            value="• `/model` – show current model and supported options",
            inline=False,
        )
        embed.add_field(
            name="Difficulty & Models",
            value=(
                "Each question rotates through easy/medium/hard difficulty and is generated by a random model. "
        ...
        embed.add_field(
            name="Need More?",
            value=(
                "Admins can use `/set_daily_channel`, `/set_admin_role`, `/reset_scores`, "
                "`/set_model`, `/remove_model`, `/add_topic`, and `/remove_topic` to manage the game."
            ),
            inline=False,
        )
        return embed
    ...
    async def _send_personal_stats(
        ...
        *,
        ...
        answered_questions = db.count_answered_questions()
        unanswered_count = db.count_unanswered_questions()
        embed = self._build_personal_embed(
            member,
            profile,
            details,
            total_questions,
    ...
    def _build_personal_embed(
        ...
        unanswered_pct = (unanswered_count / total_questions * 100) if total_questions else 0.0
        question_report = (
            f"Total stored: {total_questions}\n"
            f"Answered: {answered_questions}\n"
            f"Unanswered: {unanswered_count} ({unanswered_pct:.1f}%)"
        )
        embed.add_field(name="Question Report", value=question_report, inline=False)
